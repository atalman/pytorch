...
#ifdef USE_ROCM
  constexpr auto input_size = at::native::input_size(
...
__global__ void vectorized_elementwise_kernel(int N, func_t f, array_t data) {
  ...
  if (remaining <
      io_block_work_size<io_size>()) { // if this block handles the reminder,
...
#ifdef USE_ROCM
    constexpr auto optimal_vec_size =
        calc_optimal_vec_size<vec_size, io_size>();
#else
    ...
    elementwise_kernel_helper(
        f,
        memory::policies::
            vectorized<optimal_vec_size, array_t, elems_per_thread<io_size>()>(
                data));
  }
}
...
__global__ void unrolled_elementwise_kernel(
  ...
  auto policy = memory::policies::unroll<
      ...
      loader_t,
      storer_t,
      elems_per_thread>(data, remaining, ic, oc, l, s);
  elementwise_kernel_helper(f, policy);
}
...
static inline void launch_vectorized_kernel(
  ...
  constexpr auto io_size = calc_io_size<func_t>();
  int64_t grid =
      (N + io_block_work_size<io_size>() - 1) / io_block_work_size<io_size>();
